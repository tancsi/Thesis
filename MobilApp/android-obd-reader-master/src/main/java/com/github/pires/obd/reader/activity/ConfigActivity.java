...
import com.github.pires.obd.reader.config.FetchTeacherTask;
import com.github.pires.obd.reader.config.FetchTeachersCallback;
import com.github.pires.obd.reader.config.FetchUsersCallback;
import com.github.pires.obd.reader.config.FetchUsersTask;
import com.github.pires.obd.reader.config.ObdConfig;
import com.github.pires.obd.reader.config.Teacher;
import com.github.pires.obd.reader.config.User;
 ...
 */
public class ConfigActivity extends PreferenceActivity implements OnPreferenceChangeListener, FetchUsersCallback, FetchTeachersCallback {
    ...
    public static final String TEACHER_ID_KEY = "teacher_id_preference";
    ...
    ListPreference listUsers;
    ListPreference listTeachers;
    /**
     * @param prefs
     * @return
     */
     ...
     */
        ...
        listUsers = (ListPreference) getPreferenceScreen()
                .findPreference(VEHICLE_ID_KEY);
        listTeachers = (ListPreference) getPreferenceScreen()
                .findPreference(TEACHER_ID_KEY);
     ...
     */
        ...
        listUsers.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                int index = listUsers.findIndexOfValue(newValue.toString());
                if (index >= 0) {
                    listUsers.setSummary(listUsers.getEntries()[index]);
                    Intent resultIntent = new Intent();
                    int tripsNum=Integer.parseInt (newValue.toString().split("_")[1]);
                    resultIntent.putExtra("totalTrips",tripsNum);
                    resultIntent.putExtra("userId",newValue.toString());
                    setResult(RESULT_OK, resultIntent);
                }
                return true;
            }
        });
        listTeachers.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                return true;
            }
     ...
     */
        ...
        new FetchTeacherTask(this).execute();
    ...
    @Override
    public void setResults(ArrayList<User> users) {
        CharSequence[] userEntries = new CharSequence[users.size()];
        CharSequence[] userValues = new CharSequence[users.size()];

        for (int i = 0; i < users.size(); i++) {
            User user = users.get(i);
            userEntries[i] = user.getFirstName() + " " + user.getLastName()+" ("+user.getUserId()+")";
            userValues[i] = user.getUserId()+"_"+user.getTotalTrips();
        }

        listUsers.setEntries(userEntries);
        listUsers.setEntryValues(userValues);
    }

    @Override
    public void setResultsTeachers(ArrayList<Teacher> teachers) {
        CharSequence[] teacherEntries = new CharSequence[teachers.size()];
        CharSequence[] teacherValues = new CharSequence[teachers.size()];

        for (int i = 0; i < teachers.size(); i++) {
            Teacher teacher = teachers.get(i);
            teacherEntries[i] = teacher.getFirstName() + " " + teacher.getLastName()+" ("+teacher.getUserName()+")";
            teacherValues[i] = teacher.getTeacherID();
        }

        listTeachers.setEntries(teacherEntries);
        listTeachers.setEntryValues(teacherValues);
    }
}
